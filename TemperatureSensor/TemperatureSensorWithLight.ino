...
#define aref_voltage 5.10  //(change it according to the reference voltage captured with multimeter between =ve and -ve terminal of arduino)
...
void loop(void) {
  ...
  if(temperatureC <15){
    ...
    redValue = 66;  // off
    grnValue = 235;        // Green from full to off
    bluValue = 244;             // Blue from off to full
  
  }else  if(temperatureC >40){

    redValue = 1;  // on
    grnValue = 255;        // Green from full to off
